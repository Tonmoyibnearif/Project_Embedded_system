...
df_training = dataset.loc[dataset['Date'] <= split_date]
df_test = dataset.loc[dataset['Date'] > split_date]

df_test.drop('Date', axis=1, inplace=True)
df_training.drop('Date', axis=1, inplace=True)
test=df_test.iloc[:, 19:]
...
regressor.fit(x_train,y_train,epochs=160,batch_size=15)
